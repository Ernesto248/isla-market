...
export default function EditProductWithVariantsPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const handleHasVariantsChange = (checked: boolean) => {
    // Advertencia si está cambiando de modo
    if (initialHasVariants && !checked) {
      const confirm = window.confirm(
        "¿Estás seguro de desactivar las variantes? Esto NO eliminará las variantes existentes, pero el producto se comportará como producto simple."
      );
      if (!confirm) return;
    }
    ...
    if (!initialHasVariants && checked) {
      const confirm = window.confirm(
        "¿Deseas activar variantes para este producto? Los campos de precio y stock del producto principal se ignorarán."
      );
      if (!confirm) return;
    }
    ...
    if (hasVariants) {
      ...
      if (variants.length > 0) {
        ...
        if (duplicateCombinations.length > 0) {
          toast.error("Hay combinaciones de atributos duplicadas");
          return false;
        }
      }
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Si tiene variantes y hay cambios, actualizarlas
      if (hasVariants && variants.length > 0) {
        // Nota: En este caso simplificado, el usuario debe gestionar variantes
        // individualmente desde el VariantEditor. Para una implementación completa,
        // aquí se podría comparar con initialVariants y hacer updates/creates/deletes

        toast.warning(
          "Producto actualizado. Los cambios en variantes deben guardarse individualmente desde el editor."
        );
      } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            {hasVariants &&
              ...
              initialVariants.length > 0 && (
                <Alert>
                  ...
                  <AlertDescription>
                    Este producto tiene {initialVariants.length} variante(s)
                    existente(s). Puedes editarlas o agregar nuevas desde el
                    editor de variantes.
                  </AlertDescription>
                </Alert>
              )}
            ...
            {hasVariants && !initialHasVariants && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  Estás convirtiendo un producto simple en producto con
                  variantes. El precio y stock del producto se ignorarán. Debes
                  crear las variantes.
                </AlertDescription>
              </Alert>
            )}
            ...
            {!hasVariants && initialHasVariants && (
              <Alert variant="destructive">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  Has desactivado las variantes. El producto se comportará como
                  producto simple. Las variantes existentes NO se eliminarán,
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {hasVariants
                ? "Imágenes del producto. Se compartirán entre todas las variantes."
